...
export default function News() {
  ...
  const subcategoriesMap: Record<string, Array<{ id: string; label: { ar: string; en: string } }>> = {
    news: [
      { id: 'all', label: { ar: 'كل الأخبار', en: 'All News' } },
      { id: 'local_news', label: { ar: 'أخبار محلية', en: 'Local News' } },
      { id: 'international_news', label: { ar: 'أخبار دولية', en: 'International News' } },
      { id: 'press_house_news', label: { ar: 'أخبار بيت الصحافة', en: 'PressHouse News' } },
    ],
    report: [
      { id: 'all', label: { ar: 'كل التقارير', en: 'All Reports' } },
      { id: 'human_rights', label: { ar: 'حقوق الإنسان', en: 'Human Rights' } },
      { id: 'press_freedom', label: { ar: 'حرية الصحافة', en: 'Press Freedom' } },
      { id: 'investigative_reports', label: { ar: 'تحقيقات صحفية', en: 'Investigative Reports' } },
    ],
    press_release: [
      { id: 'all', label: { ar: 'كل البيانات', en: 'All Releases' } },
      { id: 'advocacy_statements', label: { ar: 'بيانات المناصرة', en: 'Advocacy Statements' } },
      { id: 'periodic_statements', label: { ar: 'بيانات دورية', en: 'Periodic Statements' } },
      { id: 'urgent_appeals', label: { ar: 'نداءات عاجلة', en: 'Urgent Appeals' } },
    ],
  };
  ...
  const getCategoryLabel = (cat: string) => {
    const labels: Record<string, { ar: string; en: string }> = {
      ...
      press_release: { ar: 'بيانات صحفية', en: 'Press Releases' },
    };
    return labels[cat]?.[isRtl ? 'ar' : 'en'] || cat;
  };

  const getSubcategoryLabel = (sub: string) => {
    const subLabels: Record<string, { ar: string; en: string }> = {
      local_news: { ar: 'أخبار محلية', en: 'Local News' },
      international_news: { ar: 'أخبار دولية', en: 'International News' },
      press_house_news: { ar: 'أخبار بيت الصحافة', en: 'PressHouse News' },
      human_rights: { ar: 'حقوق الإنسان', en: 'Human Rights' },
      press_freedom: { ar: 'حرية الصحافة', en: 'Press Freedom' },
      investigative_reports: { ar: 'تحقيقات صحفية', en: 'Investigative Reports' },
      advocacy_statements: { ar: 'بيانات المناصرة', en: 'Advocacy Statements' },
      periodic_statements: { ar: 'بيانات دورية', en: 'Periodic Statements' },
      urgent_appeals: { ar: 'نداءات عاجلة', en: 'Urgent Appeals' },
    };
    return subLabels[sub]?.[isRtl ? 'ar' : 'en'] || '';
  };
  ...
  const filteredArticles = articles.filter(article => {
    ...
    const matchesSub = activeSubcategory === 'all' || !article.subcategory || article.subcategory === activeSubcategory;
    ...
    <div className="min-h-screen bg-slate-50/50 pb-32">
      ...
      <section className="container mx-auto px-4 md:px-6 -mt-6 md:-mt-12 relative z-20">
        ...
        >
          ...
          {category !== 'all' && subcategoriesMap[category] && (
            <div className="mt-6 pt-6 border-t border-slate-100 flex items-center gap-2 flex-wrap justify-center lg:justify-start">
              <div className="text-[10px] font-black uppercase tracking-wider text-slate-400 mr-2">
                {isRtl ? 'التصنيف الفرعي:' : 'Sub-category:'}
              </div>
              {subcategoriesMap[category].map((sub) => (
                <button
                  ...
                  className={cn(
                    "px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer",
                    activeSubcategory === sub.id
                      ? "bg-blue-50 text-blue-600 border border-blue-200"
                      : "bg-white text-slate-500 border border-slate-200/60 hover:bg-slate-50"
                  )}
                >
                  {sub.label[isRtl ? 'ar' : 'en']}
                </button>
              ))}
            </div>
          )}
              ...
              >
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 md:top-6 md:left-6 rtl:left-auto rtl:right-4 md:rtl:right-6">
                    <div className="bg-white/90 backdrop-blur-md text-blue-900 text-[10px] uppercase font-black px-3.5 py-1.5 md:px-4 md:py-2 rounded-full shadow-xl flex items-center gap-1.5">
                      <span>{getCategoryLabel(article.category)}</span>
                      {article.subcategory && (
                        <>
                          <span className="text-slate-300">/</span>
                          <span className="text-blue-600">{getSubcategoryLabel(article.subcategory)}</span>
                        </>
                      )}
                    </div>
                  </div>
